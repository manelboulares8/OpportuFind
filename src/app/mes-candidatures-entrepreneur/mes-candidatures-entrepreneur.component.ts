...
@Component({
  selector: 'app-mes-candidatures-entrepreneur',
  standalone: false,
  templateUrl: './mes-candidatures-entrepreneur.component.html',
  styleUrl: './mes-candidatures-entrepreneur.component.css'
})
export class MesCandidaturesEntrepreneurComponent {
...
downloadCV(cvUrl: string | undefined): void {
  if (!cvUrl) {
    this.toastr.warning('Aucun CV disponible', 'Attention');
    return;
  }
  
  const link = document.createElement('a');
  link.href = cvUrl;
  link.download = this.getFileName(cvUrl);
  link.target = '_blank';
  link.click();
}
...
getFileName(url: string | undefined): string {
  ...
  return url.split('/').pop()?.split('\\').pop() || 'CV.pdf';
}
